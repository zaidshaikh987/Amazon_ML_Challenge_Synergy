...
class OptimizedEmbeddingsExtractor:
    ...
    def __init__(self):
        ...
        if TRANSFORMERS_AVAILABLE:
            try:
                ...
                logger.info("Loaded openai/clip-vit-base-patch32")
            except Exception as e:
    ...
    def extract_embeddings_batch(self, df: pd.DataFrame, image_dir: str, 
        ...
        Returns:
            tuple: (text_embeddings, image_embeddings, metadata)
        """
        logger.info(f"Extracting embeddings for {len(df)} samples")
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="Extracting embeddings"):
            ...
            if os.path.exists(image_path):
                ...
                image_emb = self._extract_fallback_image_features(image_path)
        ...
        metadata = {
            ...
            'model_info': {
                'text_model': 'sentence-transformers/all-MiniLM-L12-v2' if self.text_model else 'fallback',
