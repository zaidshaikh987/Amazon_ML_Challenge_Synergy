...
class FeatureExtractor:
    ...
    def extract_all_features(self, df: pd.DataFrame, text_col: str = 'catalog_content', image_dir: str = None) -> pd.DataFrame:
        """
        ...
        Args:
            ...
            image_dir: Optional directory to check for image files
        ...
        # Check if images exist (critical feature!)
        if image_dir and 'sample_id' in df.columns:
            features_df['has_image'] = features_df['sample_id'].apply(
                lambda sid: os.path.exists(os.path.join(image_dir, f"{sid}.jpg"))
            )
            logger.info(f"Images available: {features_df['has_image'].sum()}/{len(features_df)}")
        else:
            features_df['has_image'] = False
...
def load_embeddings(embeddings_dir: str, model_name: str = 'optimized') -> Tuple[np.ndarray, Dict]:
    ...
    """
    # Use simple prefix for simple model
    prefix = 'simple_' if model_name == 'simple' else ''
    embeddings_file = os.path.join(embeddings_dir, f'{prefix}text_embeddings.npy')
    metadata_file = os.path.join(embeddings_dir, f'{prefix}metadata.pkl')
...
def load_image_embeddings(embeddings_dir: str, model_name: str = 'optimized') -> Optional[np.ndarray]:
    ...
    # Use simple prefix for simple model
    prefix = 'simple_' if model_name == 'simple' else ''
    image_embeddings_file = os.path.join(embeddings_dir, f'{prefix}image_embeddings.npy')
...
def main():
    ...
    # Create feature matrix
    feature_matrix, feature_columns = create_feature_matrix(features_df, args.embeddings_dir, args.model)
